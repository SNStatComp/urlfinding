...
class Search:
    ...
    def _process(self,
        ...
        try:
            ...
            print(
                f"{self.population_path} is missing. Set the `input_urls_path` parameter to a csv with companies to create this file.")
            raise e
    ...
    def search(self, search_engine_config_path: str, blacklist_path: str, nrows: int, input_urls_path: str = None):
        ...
        For example if for every enterprise 6 queries are performed,
        then for 10 enterprises 6 * 10 = 60 queries are performed.
        Every query returns at most 10 search results.

        Returns:
        This function creates a file (<YYYYMMDD>searchResult.csv) in the 'data' folder containing the search results,
